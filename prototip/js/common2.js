...
//****************** страница SERVISES AJAX и JSON*********************

//Проверяем, подключаем AJAX
...
//Проверяем, подключаем JSON
	function loadTimetable() {                    // Declare function
	    $.getJSON('data/descrip.json')              // Try to collect JSON data
...
//При клике на ссылки подгружаем информацию с JSON файла
	...
	$('#content').on('click', '#event a', function(e) {  // User clicks on event
	    ...
	    for (var i = 0; i < times[loc].length; i++) {      // looping through events
	      //newContent += '<li>' + times[loc][i].number + '. ';
	      newContent += '<li><a href="data/descriptions.html#';
	      newContent += times[loc][i].descr + '">';
	    ...
	    $('#choice').text('Выберете услугу');                             // Clear third column
    });

// CLICK ON A SESSION TO LOAD THE DESCRIPTION
//При клике на ссылки, полученные с JSON файла, загружается описание c соответствующим id с html файла descriptions.html
